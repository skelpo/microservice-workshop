...
final class AddressesController: RouteCollection {
    ...
    func boot(router: Router) {
        ...
        router.post(AddressInput.self, at: "", use: create)
        router.patch(AddressInput.self, at: [Int.parameter], use: update)
    ...
    func create(_ request: Request, _ content: AddressInput)throws -> Future<UserSuccessResponse> {
    ...
    func update(_ request: Request, _ content: AddressInput)throws -> Future<UserSuccessResponse> {
        let user = try request.user()
        
        let id = try request.parameters.next(Int.self)
        let userId = try user.requireID()
        
        return Address.query(on: request).filter(\.id == id).filter(\.userId == userId).all().flatMap(to: UserSuccessResponse.self) { addresses in
    ...
    func delete(_ request: Request)throws -> Future<HTTPStatus> {
        ...
        return Address.query(on: request).filter(\.id == id).filter(\.userId == userId).all().flatMap(to: HTTPStatus.self) { addresses in
            if addresses.count == 0 {
                throw Abort(.badRequest, reason: "No address found!")
            }
